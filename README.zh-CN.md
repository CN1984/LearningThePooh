...
* Chrome （推荐）
  ...
  * 离线安装：在地址栏输入 `chrome://extensions` 并回车，拖动 `*.crx` 文件到页面内。
* Firefox
  * [Firefox 附加组件](https://addons.mozilla.org/zh-CN/firefox/addon/%E5%AD%A6%E4%B9%A0%E5%B0%8F%E7%86%8A)
  * 离线安装：在地址栏输入 `about:addons` 并回车，拖动 `*.xpi` 文件到页面内。
* QQ 浏览器
  * 离线安装：在地址栏输入 `qqbrowser://extensions/manage` 并回车，拖动 `*.crx` 文件到页面内。
* 360 安全浏览器
  * 离线安装：在地址栏输入 `se://extensions` 并回车，拖动 `*.crx` 文件到页面内。
* 360 极速浏览器
  * 离线安装：在地址栏输入 `chrome://myextensions/extensions` 并回车，拖动 `*.crx` 文件到页面内。
